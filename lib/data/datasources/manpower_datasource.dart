...
class ManpowerDatasource {
  ...
  Future<void> updateUserStatus(String nrp, bool isActive) async {
    try {
      await client.from('manpower').update({'active': isActive}).eq('nrp', nrp);
    } catch (e) {
    ...
    try {
      await client
          ...
          .update({
            ...
            'updated_at': TimeHelper.now().toIso8601String(),
          })
          .eq('nrp', user.nrp!);
    } catch (e) {
  ...
  Future<void> unregisterUser(String nrp) async {
    try {
      await client
          .from('manpower')
          .update({'registered': false, 'active': false})
          .eq('nrp', nrp);
    } catch (e) {
  ...
  Future<void> insertManpower(String nrp) async {
    try {
      final sanitized = nrp.trim().toUpperCase().replaceAll(
        RegExp(r'[^A-Z0-9]'),
        '',
      );
